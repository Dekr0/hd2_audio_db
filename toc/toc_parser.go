...
func ExtractBank(extractBankFlag string) {
	...
	for _, gameArchiveID := range gameArchiveIDs {
        ...
        if err != nil {
            log.DefaultLogger.Error("Failed to open ToC File", "error", err)
            continue
        }
        defer tocFile.Close()
		...
		if err != nil {
			log.DefaultLogger.Error("Failed to parse ToC File", "error", err)
            continue
		}
		
		for id, bank := range banks {
			if err = bank.exportToCHeader(); err != nil {
				log.DefaultLogger.Error("Failed to export ToC header", "ToCFileID", id, 
				"error", err)
			}
			if err = bank.ExportWwiserXML(false); err != nil {
				log.DefaultLogger.Error("Failed to generate Wwiser XML", "ToCFileID", id,
				"error", err)
			}
		}
	}
...
func extractWwiseStream(tocFile os.File) (map[uint64]Empty, error) {
	...
	log.DefaultLogger.Debug("ToCStart Checkpoint",
		"Head", ToCStart,
	)
	...
	for i := 0; i < int(ToC.NumFiles.Value); i++ {
		...
		log.DefaultLogger.Debug("Head change",
			"Before", ToCPrev,
			"After", reader.head,
			"Diff", reader.head - ToCPrev,
		)
        log.DefaultLogger.Info("Parsed ToC Header File ID", "fileID", header.FileID)

		if header.TypeID.Value == TYPE_WWISE_STREAM {
			if _, in := streams[header.FileID.Value]; in {
				log.DefaultLogger.Warn("A Wwise stream with duplicated ToC file ID",
...
func ExtractWwiseSoundbanks(tocFile os.File, rawData bool) (
	...
	log.DefaultLogger.Debug("ToC starting checkpoint",
		"Head", ToCStart,
	)
    ...
    for i := 0; i < int(ToC.NumFiles.Value); i++ {
        ...
        log.DefaultLogger.Debug("Parsed ToC Header File ID", 
	 ...
	 * put in out of order
	 * */
	 log.DefaultLogger.Debug("Wwise Soundbank extraction result", 
     "NumBanks", len(banks),
	 ...
	 for id, bank := range banks {
		 ...
		 if !in {
			 log.DefaultLogger.Warn("A Wwise Soundbank without a Wwise Soundbank " +
...
func parseToCBasic(reader *StingRayAssetReader) (*ToCFile, error) {
	...
	log.DefaultLogger.Debug("Readed MAGIC", 
	...
	log.DefaultLogger.Debug("Readed NumTypes", 
	...
	log.DefaultLogger.Debug("Readed NumFiles", 
	...
	log.DefaultLogger.Debug("Readed Unknown",
		"Unknown", ToC.Unknown,
		"Head", reader.head,
	)
	...
	log.DefaultLogger.Debug("Readed Unk4Data", 
		"Unk4Data", ToC.Unk4Data,
		"Head", reader.head,
	)

	log.DefaultLogger.Debug("Basic Header Checkpoint",
		"Head", reader.head,
	)
